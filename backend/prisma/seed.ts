...
function randomWalk(value: number, instrument: SeedInstrument): number {
  ...
  return Number(clamp(value + delta, instrument.min, instrument.max).toFixed(4));
}
...
async function seedInstrument(instrument: SeedInstrument): Promise<void> {
    ...
    if (!skipMinute) {
      ...
      for (let index = 0; index < updatesInMinute; index += 1) {
        ...
        const eventTime = cursor.plus({
          seconds: updatesInMinute === 1 ? Math.floor(Math.random() * 50) : 10 + index * 35,
        });
        rows.push({
          symbol: instrument.symbol,
          type: instrument.type,
            ...
            instrument.type === 'INDEX'
              ? {
                  ...
                  percentage_change_from_yesterday_close_value:
                    ((value - instrument.yesterdayClose) / instrument.yesterdayClose) *
                    100,
                }
              : {
  ...
  console.log(`Seeded ${rows.length} ticks for ${instrument.symbol}.`);
}
